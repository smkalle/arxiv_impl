...
| `leaf-taskpilot-2609.07925/` | [Leaf + TaskPilot (arXiv:2609.07925)](https://arxiv.org/abs/2609.07925) | **LeafCX** — the paper's five-tool Leaf harness and TaskPilot learnability-frontier curriculum, retargeted from SWE tasks to a self-serve used-car account-management surface. Account state is JSON the agent edits; reward is binary hidden pytest over the final state; six scenario families (shortlist, budget change, finance pre-qual, trade-in, test drive, consent) each with a reference solution. `gold`/`noop` backends bracket the reward with no model loaded; Ollama and llama.cpp (Termux) backends for real runs | v0.1.0; 167 tests pass; gold 24/24 and noop 0/24 verified |
