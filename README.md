...
| `lfm25-grpo-ifstruct/` | [Fine-tuning a 350M Model for Better Structured Outputs in 100 GRPO Steps](https://huggingface.co/blog/grpo-with-trl-ifstruct) (HF blog, not arXiv) | Hands-on Colab notebook: GRPO-tune `LiquidAI/LFM2.5-350M` for schema-compliant JSON/YAML against the [IFStruct](https://github.com/Liquid4All/ifstruct) benchmark; LoRA + TRL `GRPOTrainer`, 7 staged rewards derived from Liquid's official validator so reward == metric, synthetic 500-row train-set generator, before/after eval with a paired sign test | Notebook complete; CPU cells verified against the real 2,000-row test set (training needs a T4) |
